...
if [[ "$arch" != "aarch64" && "$arch" != "arm" ]]; then
    echo "Mi-Tool does not support architecture $arch"
    exit 1
fi
...
curl -s "https://raw.githubusercontent.com/adhit21/Android-Mod-Inject/master/mt/mi-tool.py" -o "$PREFIX/bin/mi-tool" && chmod +x "$PREFIX/bin/mi-tool"
...
if [ ! -f "$miunlockusers" ]; then
    ...
    curl -sSL -o "$PREFIX/bin/miunlock" https://raw.githubusercontent.com/adhit21/mu/main/MiUnlockTool.py
...
curl -s "https://raw.githubusercontent.com/adhit21/Android-Mod-Inject/master/mt/MiBypassTool.py" -o "$PREFIX/bin/mibypass" && chmod +x "$PREFIX/bin/mibypass"


_progress

curl -s -L -o $PREFIX/bin/miasst_termux $(curl -s "https://api.github.com/repos/adhit21/ma/releases/latest" | grep "browser_download_url.*miasst_termux_${arch}" | cut -d '"' -f 4) && chmod +x $PREFIX/bin/miasst_termux
